...
export default function ContactPage() {
  ...
  const [validationErrors, setValidationErrors] = useState<{type?: string, content?: string}>({})
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    else if (content.length > 1000) newErrors.content = '1000文字以内で入力してください。'
  ...
  return (
    <div className="max-w-xl mx-auto py-8 sm:py-12 animate-fade-in">
      <div className="mb-8 px-4 sm:px-0">
        <button onClick={() => router.back()} className="text-emerald-400 hover:text-emerald-300 transition-colors flex items-center gap-2 text-sm font-medium">
      ...
      <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-3xl p-6 sm:p-10 shadow-2xl mx-4 sm:mx-0">
        <h1 className="text-2xl font-black text-white mb-2 tracking-tight">お問い合わせ</h1>
          ...
          <form onSubmit={handleSubmit} className="space-y-6" noValidate>
            <div className="space-y-1.5">
              ...
              <div className="relative">
                ...
                >
                  <option value="" disabled className="text-gray-500">選択してください</option>
                  <option value="機能要望" className="bg-gray-900 text-white">機能のご要望</option>
                  <option value="不具合報告" className="bg-gray-900 text-white">不具合のご報告</option>
                  <option value="アカウントに関するお問い合わせ" className="bg-gray-900 text-white">アカウントに関する内容</option>
                  <option value="その他" className="bg-gray-900 text-white">その他のお問い合わせ</option>
                </select>
              ...
              {validationErrors.type && (
                <p className="text-red-400 text-xs mt-1 ml-1 animate-fade-in">{validationErrors.type}</p>
              )}
            </div>
            ...
            <div className="space-y-1.5">
              ...
              {validationErrors.content && (
                <p className="text-red-400 text-xs mt-1 ml-1 animate-fade-in">{validationErrors.content}</p>
              )}
            </div>
