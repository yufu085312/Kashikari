...
export const api = {
  ...
  getGroup: (groupId: string) =>
    request<{ id: string; name: string; members: { id: string; name: string }[] }>(
      `/groups/${groupId}`
    ),
  ...
  getBalances: (groupId: string) =>
    request<{ fromUserId: string; toUserId: string; amount: number; fromUserName?: string; toUserName?: string }[]>(
