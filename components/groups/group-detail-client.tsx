...
export function GroupDetailClient({ 
  ...
  const [activeTab, setActiveTab] = useState<'balance' | 'payment' | 'settlement'>('balance')
  ...
  const inviteUrl = typeof window !== 'undefined' ? `${window.location.origin}/groups/${groupId}/invite` : ''
  ...
  const latestSettlementAt = settlements.length > 0 
    ? settlements[0].created_at 
    : null
  ...
  const handleDeleteGroup = async () => {
    ...
    const isConfirmed = await confirm({
      title: 'グループの削除',
      message: 'このグループを完全に削除してもよろしいですか？\nこの操作は取り消せません。',
      type: 'danger',
      confirmText: '削除する',
      cancelText: '戻る'
    })
      ...
      <div className="min-h-[400px]">
          ...
          <div className="space-y-6 animate-slide-up">
            <section>
              <div className="flex items-center justify-between mb-4 px-1">
                <h2 className="text-xs font-black text-gray-500 uppercase tracking-widest">支払い履歴</h2>
        ...
        {activeTab === 'settlement' && (
          <div className="space-y-6 animate-slide-up">
            <section>
              <div className="flex items-center justify-between mb-4 px-1">
                <h2 className="text-xs font-black text-gray-500 uppercase tracking-widest">直近の精算</h2>
                <div className="h-px flex-1 bg-white/5 ml-4"></div>
              </div>
              <SettlementList 
      ...
      >
        <div className="space-y-6">
          <div className="space-y-3">
            <label className="text-xs font-bold text-gray-400 uppercase tracking-wider">検索IDで追加</label>
            ...
            {addMemberError && (
              <p className="text-[10px] text-red-500 font-bold">{addMemberError}</p>
            )}
          </div>
          ...
          <div className="space-y-3">
            <label className="text-xs font-bold text-gray-400 uppercase tracking-wider">招待リンクを共有</label>
            <div className="flex gap-2">
              <input
                type="text"
                readOnly
